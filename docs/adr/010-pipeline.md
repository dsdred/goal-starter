...
- Two `Active` pipelines referencing the same model: the **earlier pipeline (repository order)** launches and owns the instance; the later pipeline's entry yields `already-running`.
