...
- Models: chips as today; repeated model rendered **once per entry with an occurrence badge** (`Qwen3.8-27B` `Qwen3.8-27B` with a shared `×2` badge style) — each chip keeps its own state color (per-entry state, D5); chips beyond a display budget collapse to `+N` with a tooltip listing the remaining entries (RU/EN). **Shipped implementation (2026-09-09 owner-UI refinement, `3347b2b`): one name-chip per entry, each with its own per-entry state color — the shared ×N occurrence badge from this proposal was not shipped.**
...
The per-entry autostart semantics are **preserved unchanged** (ADR 010 D1/D4: a pipeline starts on GoAl startup only if `Active = true`; within that, only entries with `AutoStart = true` launch; `AutoStart = false` means "no automatic launch" only — manual Start processes **all** entries). This ADR removes **no** autostart semantics and changes **none** of them. **Superseded for the shipped build by the 2026-09-09 owner-UI reconciliation (shipped `3347b2b`, released in v2.1.0): an Active pipeline launches ALL entries; the per-entry `AutoStart` field is written-but-ignored (retained for storage/API backward compatibility) and is not consulted for launch selection — see the Status line.**
