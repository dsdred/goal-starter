...
**Status:** Accepted (design gate 2026-09-06; owner contract agreed 2026-09-06 — D1, D3, D5, D6, D7 + release scope). **UI-Acceptance refinement (owner decision 2026-09-07):** the rejected form/table create-edit editor is replaced by a **Scratch-inspired linear builder** (vertical connected blocks; not a canvas/DAG/DnD), list secondary actions move into a **"…" overflow menu** (single primary Start/Stop + overflow), and desktop columns are ordered **Name | Models | State | Autostart | Primary+overflow** — a UI/UX refinement of D7 with **no** change to lifecycle/entry-identity/schema v8/API/autostart semantics. **Implementation complete 2026-09-07** (uncommitted working tree) — all D10 acceptance items pass (Go: application/storage/handlers/cmd-goal autostart matrix + recovery of two instances of one model; maintained browser suite `npm test`: 9 suites all green, `pipeline.cjs` 105/105 after the 2026-09-08 Owner UI Acceptance correction added bounded connector-geometry + no-leaked-generated-source regression guards for 1/2/3 blocks, duplicate Model ×2, and desktop/mobile). Gates: `gofmt`/`go vet` clean, `go test ./...` green, Windows build pass, race via Linux CI. Awaiting the commit gate and the separate release authorization (no commit / publish / tag / release performed).
...
- Primary action: **one** prominent button — `Start` when no owned active instances of the pipeline exist, `Stop` when at least one does (lifecycle permits exactly one sensible primary action; both never shown as equals).
- Secondary actions: `Restart`, `Edit`, `Delete` in a **"…" overflow menu** (`Restart`/`Edit`/`Delete` items, `Delete` destructive inside the menu, each with RU/EN label + `aria-label`), so they never compete with the single primary Start/Stop; the ambiguous "logs" icon (`viewPipelineLogs` → plain navigate to the global Logs page) is **removed** (logs remain reachable from the Logs page's instance selector and from per-instance log actions).

**Mobile (L):** the existing table→cards contract at 768px is kept (single monotonic breakpoint, no new breakpoints). The compact card gains semantic hierarchy: **name (ellipsis+title) → model chips → labeled Autostart switch → primary Start/Stop → secondary actions in an overflow menu ("…")** so controls keep their meaning after the table headers disappear.
