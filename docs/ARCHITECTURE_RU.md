...
| 0011 | Windows Service / Background Mode: true-интеграция с SCM (Accepted 2026-08-31; реализовано 2026-08-31 — in-binary `--service`-глаголы, `--service run` как внутренний SCM-entrypoint, абсолютная зарегистрированная команда, absolute-path pre-flight install, LocalSystem, `auto`-старт без имплицитного запуска, StopPending/30 s wait hint + внешний бюджет 45 s, SCM-level restart, Event Log операционные диагностики (slog-хендлер), ps1-скрипты и ссылки в MSI-тулинге удалены в том же изменении) | Implemented |
| 0013 | Pipeline: повторное использование модели + UX/product polish (Accepted 2026-09-06, owner contract agreed 2026-09-06; **owner-UI reconciliation 2026-09-09** — entry-level identity `PipelineModel.id` (supersedes ADR 010 D1.3), per-entry launch gate + model-owner rule (supersedes ADR 010 D3, расширяет D4), per-entry stop/restart с legacy-fallback, additive API `entry_id`/`index`, без bump схемы (v8), строгая Args-семантика неизменна, **`AutoStart` reconciled: Active → все записи запускаются (per-entry поле written-but-ignored; UI-контрол + ⚡ удалены)**, UI-контракт: явный Args-режим, ↑/↓/× reorder, inline icon strip (без overflow "…"), bounded grid form-head, mobile dense card, instances geometry, набор иконок sidebar, переименование History, empty state, responsive polish) | Accepted |
| 0014 | Portable Configuration — Variable Resolution и Secret-Safe Export/Import (Accepted 2026-09-13; синтаксис `${NAME}` + `$$` escape, built-in `GOAL_DATA`, процесс-окружение как источник, resolve-at-consumption, ошибки undefined-var + malformed-ref, secret-safe экспорт (только ключи), portable bundle v1, атомарный graph-import, 3-slice phasing; **MVP завершён — Slices 1–3 опубликованы 2026-09-15**, финальный SHA `4624d81`, CI 7/7 PASS; политика конфликтов импорта **амедирована 2026-09-25: SKIP EXISTING** вместо исходной REJECT-коллизии) | Accepted |
