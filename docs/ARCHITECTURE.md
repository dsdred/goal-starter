...
| 0014 | Portable Configuration — Variable Resolution and Secret-Safe Export/Import (Accepted 2026-09-13; `${NAME}` syntax + `$$` escape, built-in `GOAL_DATA`, process-env source, resolve-at-consumption, undefined-var + malformed-ref errors, secret-safe export (keys only), portable bundle v1, REJECT collision, atomic graph-import, 3-slice phasing; implementation NOT STARTED) | Accepted |
