...
После перезапуска GoAl ранее активный экземпляр, чей процесс, возможно, всё ещё выполняется вне GoAl, отображается как **ORPHAN** («Может выполняться вне GoAl»). Доступны два действия:
