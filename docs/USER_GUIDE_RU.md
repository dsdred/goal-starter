...
GoAl — лёгкий кроссплатформенный менеджер для локальных AI-рантайнов, моделей и профилей запуска. Один бинарник для Windows и Linux.
...
7. [Профили запуска](#профили-запуска)
8. [Рантаймы](#рантайны)
9. [Модели](#модели)
10. [Логи](#логи)
...
- **CRUD профилей** — создание, редактирование, удаление профилей
- **CRUD рантайнов** — настройка AI-рантайнов
- **CRUD моделей** — настройка моделей
...
| POST | `/api/v1/profiles/{id}/start` | Запустить по профилю |
...
| GET | `/api/v1/profiles/{id}/status` | Статус процессов |
| POST | `/api/v1/profiles/{id}/activate` | Активировать |
| POST | `/api/v1/profiles/{id}/deactivate` | Деактивировать |
...
Один профиль может создавать множество экземпляров. Остановка экземпляра не удаляет профиль. Перезапуск создаёт новый экземпляр.
