...
- В списке одно **основное** действие — **Старт**, если пайплайн не владеет активными
  экземплярами, и **Стоп**, если владеет, — а **Перезапуск / Редактировать / Удалить** в **«…» меню** (Удалить — деструктивный пункт в меню).
  Столбец **Состояние** агрегирует живые состояния записей (running / starting / stopping /
  failed / orphan / stopped), повторяющаяся модель отображается по разу на запись со значком
  `×n` и собственным состоянием.
