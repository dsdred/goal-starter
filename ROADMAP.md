...
- [ ] Login rate limiting: per-client-address fixed window on `POST /api/v1/auth/login` (100 requests/min, HTTP 429 `rate_limited`); `X-Forwarded-For`/`X-Real-IP` intentionally not trusted (spoofable)
