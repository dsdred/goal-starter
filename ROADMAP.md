...
- [ ] Full audit logging: structured security audit events (login, session, settings, instance actions), persistence and retention, query API. Replaces the metrics-only state; satisfies the audit commitment in ADR 005 (Dismiss audit). Significant work per DEVELOPMENT.md (new entity + persistence + public API)
  - Design gate: [ADR 007](docs/adr/007-audit-logging.md) (**Accepted** — owner contract agreed 2026-08-24, implemented 2026-08-25) defines the event taxonomy (login/session/settings/instance actions), JSONL append-only storage with per-event fsync and 3×10 MiB rotation, `GET /api/v1/admin/audit` query API, hard secret-safety rules, fail-open failure semantics (structured `slog.Error` per failed write, no secret-bearing diagnostic data, no logger latching, independent retry per event), and a 17-scenario acceptance contract
  - Implementation per ADR 007 §8 complete in the working tree (2026-08-25): `internal/webui/audit/` (AuditLogger + rotation + tests), handler call sites, `GET /api/v1/admin/audit`, 17-scenario acceptance tests (scenarios 1–13, 16–17 in `internal/webui/handlers/audit_integration_test.go`; 14–15 in `internal/webui/audit/audit_test.go`), docs updated (API.md, SECURITY.md, CONFIGURATION.md, USER_GUIDE EN/RU, ARCHITECTURE EN/RU). Pending commit/publication/CI reconciliation, after which this item moves to `[x]`
...
- [ ] Hot-reload wired into main startup
  - Pre-implementation forensic: `config.ReloadConfig` exists but is unused in production code, and its `Save()` does not follow the durable-write contract (no fsync/read-back/`.bak`); the hot-vs-restart-required field semantics (listener rebind, auth toggle, `hint` contract from the settings endpoint) must be defined before wiring
