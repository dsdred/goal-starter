...
- [ ] WebUI: SSE reconnect replays the last 1000 lines without client-side dedup — the server replays history on every (re)connect (`system.go` `PageSize: 1000`; the earlier "500" figure was stale) and the client re-appends them, so network drops can duplicate rendered lines. Manual UI/UX review 2026-08-27: duplication not reproduced in real Chromium (CDP offline emulation did not disconnect the existing SSE stream); code-path risk stands; the replay already emits the SSE `id` (sequence), so client-side dedup by sequence is straightforward
...
- [ ] WebUI: status badge crushed/overlapped by long model names in narrow layouts (430px, 150% zoom) — the model card name has no overflow constraint, the badge truncates to "ОСТАНОВ" and the name renders on top of it (verified at 430 and 1280@150%). Manual UI/UX review 2026-08-27
...
- [ ] WebUI: i18n gaps — (a) unmatched server error messages pass through untranslated (`friendlyError()` maps a fixed pattern set; login "invalid credentials" displayed raw in RU UI); (b) "Runtime" nav label untranslated in RU mode while "Экземпляры" is translated. Manual UI/UX review 2026-08-27
- [ ] WebUI: no user-visible feedback when the server connection drops — `logEs.onerror` is an empty handler, no toast/banner appears, and the sidebar "Сервер" dot is hardcoded to `online` in index.html and never updated by the client (stays green while the server is down). Per-action error toasts exist, but global connectivity state is not shown. Manual UI/UX review 2026-08-27
