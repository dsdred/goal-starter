...
- [ ] **F-2 — missing deterministic retry regression.** No test pins the sequence: repository cleanup succeeds → the reconciliation `List` fails → a retry with `deleted == 0` short-circuits → reconciliation succeeds on the retry. The behavior exists in the D3 application path; only the regression test is absent.
...
- [ ] **`docs/ARCHITECTURE.md` / `docs/ARCHITECTURE_RU.md` ADR summary tables stop at ADR 014.** The ADR 016 and ADR 017 rows promised by ADR 017's affected-documentation list are missing. Documentation debt only; no behavior implication.
