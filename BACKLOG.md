...
- [ ] **D62-01 — `docs/API.md` documents `not_restartable` by only one of its two producers (recorded 2026-09-26; from the D6.2 gate record).** `docs/API.md:118` and the `409` row at `docs/API.md:187` describe `error=not_restartable` solely as "the current launch configuration cannot be re-resolved (missing model/runtime/owning entry, or legacy attribution)". The code also returns it as a plain **state refusal**, independent of re-resolution: `checkRestartTarget` (`internal/process/arbitration.go:304-315`) reaches `default:` for any state outside `pending`/`starting`/`running`/`stopping`/`exited`/`failed` — i.e. `stale` and `orphan` — and wraps `ErrNotRestartable`, which `lifecycleAPIError` maps to `409 conflict not_restartable` (`internal/webui/handlers/helpers.go:136-137`). Both API.md statements are true but incomplete, so the row is documentation debt, not a defect: **no behavior change is required — the wording must be extended to name the state-refusal cause.** Do not fix by making `stale` restartable; that refusal is the deliberate D5-01 contract.
- [ ] **D62-02 — the `F-3` forensic note is now self-referential (recorded 2026-09-26; from the D6.2 gate record).** Two current claims assert that the token does not exist anywhere in history: the "`On the F-* numbering (recorded by D6.1)`" note at the head of this section, and the same sentence in the ROADMAP D5-disposition bullet (`ROADMAP.md:85`, under "D5 finding disposition"). Both phrase it as "`git log --all -S"F-3"` finds no such token in any tracked file or commit message on any ref". D6.1 itself (`3ba719d`) wrote the literal `F-3` into those tracked files, so the forensic command now returns that commit and **the sentence as phrased refutes itself**. The *conclusion* is unaffected and stands: no `F-3` finding was ever recorded, the series comes from an external review register, and no `F-3` defect may be invented. Required correction is wording-only — scope the claim to "no commit introduced an `F-3` **finding**" (or record the command as of the pre-D6.1 baseline `68fdd63`) — and must not delete the non-finding conclusion.
